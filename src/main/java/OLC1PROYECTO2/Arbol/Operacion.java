 ...
 * @author javie
 */
public class Operacion extends Instrucciones {
    ...
    public static enum Tipo_operacion {
        ...
        MENOR_QUE,
        CONCATENACION,
        IGUAL, DIFERENCIA, MENORIGUAL, MAYORIGUAL
    }
    /**
    ...
    public Object ejecutar(TablaDeSimbolos ts) {
        ...
        if (tipo == Tipo_operacion.DIVISION) {
            ...
            return ts.getVariable(valor.toString());
        } else if (tipo == Tipo_operacion.CADENA) {
            return valor.toString().replace("\"", "");
        } else if (tipo == Tipo_operacion.CARACTER) {
            return generarChar();
        } /* ======== OPERACIONES RELACIONALES ======== */ else if (tipo == Tipo_operacion.MAYOR_QUE) {
            ...
            return ((Double) operadorIzq.ejecutar(ts)).doubleValue() != ((Double) operadorDer.ejecutar(ts)).doubleValue();
        } else if (tipo == Tipo_operacion.MENORIGUAL) {
            return ((Double) operadorIzq.ejecutar(ts)).doubleValue() <= ((Double) operadorDer.ejecutar(ts)).doubleValue();
        } else if (tipo == Tipo_operacion.MAYORIGUAL) {
            return ((Double) operadorIzq.ejecutar(ts)).doubleValue() >= ((Double) operadorDer.ejecutar(ts)).doubleValue();
